...
(() => {
  ...
  const authRequired=()=>liveApiConfigured()&&!!(AUTH&&AUTH.configured());
  ...
  function security(){shellTitle('الهوية والتشغيل','Identity & Runtime');const i=state.identity||D.identity;return `<section class="grid section-grid"><article class="card"><div class="section-title"><h2>${t('الهوية الحالية','Current identity')}</h2>${pill(i.auth_source)}</div><dl><dt>User ID</dt><dd>${esc(i.user_id)}</dd><dt>${t('الأدوار','Roles')}</dt><dd>${(i.roles||[]).map(pill).join(' ')}</dd><dt>${t('المواقع','Sites')}</dt><dd>${esc((i.site_codes||['TGH']).join(', '))}</dd></dl><div class="session-box"><strong>${t('الجلسة','Session')}</strong><div class="muted">${AUTH&&AUTH.isAuthenticated()?t('جلسة مؤسسية نشطة عبر OIDC','Active institutional OIDC session'):(AUTH&&AUTH.configured()?t('لا توجد جلسة نشطة','No active session'):t('لم يُضبط مزود الهوية بعد','Identity provider not configured yet'))}</div>${AUTH&&AUTH.isAuthenticated()?`<button id="sessionLogout" class="btn" style="margin-top:.8rem">${t('تسجيل الخروج','Sign out')}</button>`:(AUTH&&AUTH.configured()?`<button id="sessionLogin" class="btn" style="margin-top:.8rem">${t('تسجيل الدخول','Sign in')}</button>`:'')}</div></article><article class="card"><h2>${t('بوابات الإنتاج','Production gates')}</h2><ul><li>OIDC issuer, audience and JWKS</li><li>TLS and approved CORS origin</li><li>Role and site claim mapping</li><li>PostgreSQL + MinIO backup/restore</li><li>Notification scheduler and export cleanup</li><li>UAT and Turaif go-live approval</li></ul><div class="notice Warning">${t('Dev tokens يجب تعطيلها في الإنتاج.','Development tokens must be disabled in production.')}</div></article></section>`}
  ...
  (async()=>{
    if(AUTH&&AUTH.configured()){
      try{ await AUTH.completeLogin() }
      catch(e){ showLogin(t(`فشل تسجيل الدخول: ${e.message}`,`Sign-in failed: ${e.message}`)); return }
